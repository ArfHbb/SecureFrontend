...
export default {
  ...
  methods: {
    async fetchArrivals() {
      try {
        const res = await api.get("/api/arrivals");
        // ...log dihapus demi keamanan...
        this.arrivals = res.data.data;
      } catch (err) {
        let msg = "Terjadi kesalahan saat mengambil data kedatangan. Silakan coba lagi nanti.";
    ...
    openModal(arrival) {
      ...
      this.selectedArrivalHtml = purify(`
    ...
    async approveArrival() {
      try {
        const res = await api.post(
            `/api/arrivals/${this.selectedArrival.id}/approve`,
            {},
            {
              headers: {
    ...
    async rejectArrival() {
      try {
        const res = await api.post(
            `/api/arrivals/${this.selectedArrival.id}/reject`,
            {},
            {
              headers: {
                "Content-Type": "application/json"
    ...
    logout() {
      // Hapus data login/token dari browser
